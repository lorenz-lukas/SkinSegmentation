...
base_path = '.'
test_name = 'SFA_pixel_regions_test_random_pixels_set'
...
full_gt_test_filenames = [os.path.join(gt_basefolder,tr) for tr in test_filenames]

full_ori_test_filenames = [os.path.join(ori_basefolder,tr) for tr in test_filenames]
...
total_skin = 0
f = 0
for filename in full_gt_test_filenames:
    file_skin = []
    file_back = []
...
total_examples = n_skin_examples_per_image*len(test_filenames) + n_background_examples_per_image*len(test_filenames)
...
for fidx, filename in enumerate(full_ori_test_filenames):
    ...
    for sk_idx in range(n_skin_examples_per_image):
        os.system('clear')
        print "Full_set_bigger_patch_test: %f%%" % ((float(sk_idx + 1 + fidx*(n_skin_examples_per_image+n_background_examples_per_image) )/ total_examples) * 100)
    ...
    for b_idx in range(n_background_examples_per_image):
        os.system('clear')
        print "Full_set_bigger_patch_test: %f%%" % ((float(b_idx + 1 + fidx*(n_skin_examples_per_image+n_background_examples_per_image) + n_skin_examples_per_image )/ total_examples) * 100)
